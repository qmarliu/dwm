...
arrangemon(Monitor *m)
{
	if (m->isoverview) {
		...
		overviewlayout.arrange(m); return;
	}
    ...
    if (selmon->isoverview) {
      ...
      do {
		...
		if (i < LENGTH(tags)) {
			...
			click = ClkLtSymbol;
		/* 2px right padding */
		else if (ev->x > selmon->ww - drawstatusbar(selmon) - 2)
			click = ClkStatusText;
		else {
...
view(const Arg *arg)
{
  ...
  if (selmon->isoverview && arg->i == 0) {
    selmon->isoverview = 0; // 在overview的情况下，切换tag
  }
